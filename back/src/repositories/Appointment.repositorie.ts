...
export const AppointmentModel = appDataSource.getRepository(Appointment).extend({
    ...
    validateAllowAppointment: function(date: Date, time: string): void{
        ...
        if(appointmentDateArg < nowArg) throw Error(`No se pueden agregar citas para fechas pasadas`) 
        if(dayOfWeek === 5 || dayOfWeek === 6) throw Error(`No es posible agendar citas kis fines de semana`)
        if(hours < 8 || hours > 18) throw Error(`Solo se pueden agendar citas entre las 8 am y las 6 pm`)
    },
